...
_VITL14 = dict(
    ...
    laion2b_s32b_b82k=_pcfg(
        hf_hub="laion/CLIP-ViT-L-14-laion2B-s32B-b82K/", mean=INCEPTION_MEAN, std=INCEPTION_STD
    ),
    # DataComp-XL models
...
_PRETRAINED = {
    ...
    "GeoLB-ViT-14-so400m-SigLIP-384": dict(
        webli=_slpcfg(hf_hub="earthflow/GeoLB-ViT-14-SigLIP-so400m-384/"),
    ),
...
def list_pretrained(as_str: bool = False):
    """Returns list of pretrained models Returns a tuple (model_name, pretrain_tag) by default or
    'name:tag' if as_str == True."""
    return [
