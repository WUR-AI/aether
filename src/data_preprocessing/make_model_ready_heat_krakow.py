"""Build model-ready CSV for the UHI use case - Kraków dataset
(data/heat_krakow/model_ready_heat_krakow.csv)."""
...
NUMERIC_COLS = [
    ...
    "MeanBuildingsHeight",
    "BuildingsCoverHa",
    "BuildingsCoverPercent",
    "CopernicusMeanImperviousDensity2021",
    # Socio-demographic
    "RES_per_1ha",
    # Terrain
    "dem_5m",
    ...
    "ForestCoverHa",
    "ForestCoverPercent",
    "VegetationVoxelNumber",
    "VegetationLiDARPtsNumber",
    "HVV2BV",
    "CopernicusMeanTreeCoverDensity2023",
]
...
CATEGORICAL_COLS = [
    ...
    "GridType",
    "GridDominantLanduse",
    "CopernicusCLCPlusBackbone2023Class",
    "CopernicusHerbaceousVegPresence2023",
]
...
def main(source_csv: str, out_csv: str, drop_zero_lst: bool = True) -> None:
    df = pd.read_csv(
        source_csv, encoding="windows-1250"
    )  # enforce windows-1250 encoding to handle special Polish characters in district names
    print(f"Loaded:     {source_csv}  →  {df.shape[0]} rows, {df.shape[1]} cols")

    # ------------------------------------------------------------------ #
    # 1. Clean target: drop zero and NaN LST rows                         #
    # ------------------------------------------------------------------ #
    target_col = "LST_20240829"
